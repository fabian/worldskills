...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (count($errors) == 0) {
		$stmt = $db->prepare('INSERT INTO `users` (title, firstname, lastname, email, password, company, phone, fax, cell, street, postal, city, province, confirmation, type, ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
		$key = makekey();
		...
		if ($stmt->execute(array(
			...
			$values->postal,
			$values->city,
			$values->province != '' ? $values->province : null,
			$key,
			'employer',
			$_SERVER['REMOTE_ADDR'],
		))) {
...
Please confirm your registration: http://sk17user12/employer/confirm.php?id=$id&key=$key
...
$template = new Template('employer/register.xsl');
